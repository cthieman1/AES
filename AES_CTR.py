...
def get_counter_block(iv, counter):
    counter_block = iv + counter.to_bytes(4, byteorder='big')  # total: 16 bytes
    return list(counter_block)
...
if __name__ == '__main__':
    ...
    #counter_block = list(counter_block)
    ...
    for i in range(len(byte_block_list)):
        counter_block = get_counter_block(initialization_vector, counter)
        keystream_block = Cipher(counter_block, Nr, expanded_key)  # Encrypt the counter
        cipher_block_list[i] = XOR_blocks(byte_block_list[i], keystream_block)
        counter += 1
    ...
    for i in range(len(cipher_block_list)):
        counter_block = get_counter_block(initialization_vector, counter)
        keystream_block = Cipher(counter_block, Nr, expanded_key)  # AES encrypt counter block
        plaintext_block_list[i] = XOR_blocks(cipher_block_list[i], keystream_block)
        counter += 1
