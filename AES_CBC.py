...
key = "".join(format(ord(c), '08b') for c in key_text)
...
def ReverseCipher(byte_block, Nr, key_schedule):
...
def remove_padding(decrypted_message):
    last_block = decrypted_message[-1]
    padding_length = last_block[-1]

    if padding_length == 0:
        del decrypted_message[-1]
    ...
    # Remove padding
    decrypted_message[-1] = last_block[:-padding_length]
    return decrypted_message
...
def XOR_blocks(block1, block2):
    ...
    if type(block1[0]) == list:
        block1 = [item for sublist in block1 for item in sublist]
    if type(block2[0]) == list:
        block2 = [item for sublist in block2 for item in sublist]


    for i in range(0,len(block1)):
        new_block_list.append(block1[i] ^ block2[i])
...
if __name__ == '__main__':
    ...
    iv_list = list(initialization_vector)
    ...
    for i in range(len(byte_block_list)):
        xored_block = XOR_blocks(byte_block_list[i], previous_block)
        cipher_block_list[i] = Cipher(xored_block, Nr, expanded_key)
        previous_block = cipher_block_list[i]

    cipher_block_list = [[byte for part in block for byte in part] for block in cipher_block_list]
    for i in range(0, len(cipher_block_list)):
        print(f'cipher block {i}: {cipher_block_list[i]}')
    print()

    # Assume cipher_block_list does NOT include the IV — it's stored separately
    plaintext_block_list = [None] * len(cipher_block_list)
    previous_block = iv_list  # This must be the original IV used during encryption

    for i in range(len(cipher_block_list)):
        decrypted_block = ReverseCipher(cipher_block_list[i], Nr, expanded_key)
        plaintext_block_list[i] = XOR_blocks(decrypted_block, previous_block)
        previous_block = cipher_block_list[i]  # This becomes C_{i-1} for next round
